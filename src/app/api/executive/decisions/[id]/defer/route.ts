...
export async function POST(
  ...
  try {
    ...
    const decision = await DecisionOrchestrator.deferDecision(id, user.organizationId, {
    ...
    console.error('[API /api/executive/decisions/[id]/defer POST] Error:', error);
    return NextResponse.json(
      { error: error.message || 'Failed to defer decision' },
      { status: 400 }
    );
  }
}
