...
import { grantCredits, reserveCredits, consumeCredits, refundCredits, getCreditBalance } from './src/lib/billing/credits';
...
async function run() {
  ...
  try {
    ...
    await consumeCredits(orgA.id, 30, 'DISCOVERY_JOB', 'job1', 'Consumed 30 leads');
    balA = await getCreditBalance(orgA.id);
    assert.strictEqual(balA.available, 60);
    assert.strictEqual(balA.reserved, 10); 

    console.log("7. Testing Refund...");
    await refundCredits(orgA.id, 10, 'DISCOVERY_JOB', 'job1', 'Refund unused 10 leads');
    balA = await getCreditBalance(orgA.id);
    assert.strictEqual(balA.available, 70);
    assert.strictEqual(balA.reserved, 0);

    console.log("8. Testing Idempotent Refunds...");
    await refundCredits(orgA.id, 10, 'DISCOVERY_JOB', 'job1', 'Refund unused 10 leads again');
    balA = await getCreditBalance(orgA.id);
    assert.strictEqual(balA.available, 70);
    assert.strictEqual(balA.reserved, 0);
